...
from dataset_saver import capture_and_save_features
# Function to load keys from files
from model_trainer import train_save_model
...
def register_user():
    if request.method == 'POST':
        ...
        # Load the CSV file
        file_path = "adhar_database.csv"

        # Read the CSV file
        df32 = pd.read_csv(file_path)

        # Get new Aadhaar number from user
        new_adhar_number = int(username)

        # Append the new number to the DataFrame
        df32.loc[len(df32)] = [new_adhar_number]

        # Save the updated DataFrame back to the CSV file
        df32.to_csv(file_path, index=False)

        print("Aadhaar number added successfully!")

        #process_video('register', username=username)
        capture_and_save_features(username)
        train_save_model()
        return f"User {username} registered successfully!"



@app.route('/nextpage54')
def nextpage54():
...
#def send_text():
    # Send either "yes" or "no" to the client
#   text_to_send = "king"
#    if text_to_send:
#        return text_to_send
#    else:
...
def receive_result():
    result = request.form.get('result')  # Receive result from client
...
# prepare a cursor object using cursor() method
    ...
    if gmail_list.index(logu)==password_list.index(passw):
        # Read the CSV file
            ...
            return render_template('reg_admin_face.html')    
...
#import pickle
    ...
    t = int(output1[0])  # Define the integer

    # Save the integer to a pickle file
    with open("saved_integer.pkl", "wb") as file:
        pickle.dump(t, file)
    ...
    if user_aadhar in aadhar_df['Adhar_numbers'].values:
        ...
        print("the user input is checked here  1",new_adhar_number)

        print("list of registered people",df1['adhar_number'].values)


        if   str(t) in df1['adhar_number'].values:
                   print(f"{new_adhar_number} has already voted.")
                   return render_template('login121.html',alert_text="YOU ALREADY VOTED")
        else:


            print("the user input is checked here   2",new_adhar_number)
            ...
            otp_sender(random_number)


            voted_details.append(new_adhar_number)
            voted_details.append(random_number)
...
def predict5():
    ...
    with open('temp_file.txt', 'r') as file:
        read_number = file.read()
        print("Read number:", read_number)


    if  int(read_number)==int(int_features1[0]):
                 ...
                 # resultcrop = {value:key for key, value in croplist.items()}
                  ...
                  user = process_video()
                  print(user)
                    # Load the integer from the pickle file
                  with open("saved_integer.pkl", "rb") as file:
                        loaded_t = pickle.load(file)

                  print("Loaded integer:", loaded_t)

                  if int(user)==int(loaded_t):
                               ...
                               return render_template('voting_page1.html')
                                ...
                                #return render_template('main.html',result="login success !")
...
# Delete the temporary file
    ...
    #df1 = df1.append({'adhar_number': voted_details[0], 'otp':voted_details[1], 'voted': voted_details[2],"hashed_encrypted_all_blocks":result}, ignore_index=True)
            ...
            # Save the updated DataFrame back to the CSV file
    #df1.to_csv('voting_data4.csv', index=False)
